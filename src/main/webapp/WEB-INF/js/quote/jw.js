...
(function($){
	...
	ok_text: typeof(ok_text) != 'undefined' ? ok_text : "确 定",
	alert_title: typeof(alert_title) != 'undefined' ? alert_title : "提示",
	...
	over:function(opt){
		opt=opt||'show';
		var over,bh=$('body')[0].scrollHeight;//$('body').height();
		if(opt=='show'){
	...
	position_fixed:function(target,win){
	 ...
	 drag:function(bar,target,win){
	  ...
	  dialog:function(option){
		  	//prevent the parent window event
	        ...
	        if(option.title!==false){
	              ...
	              header="<table width='100%' class='jw-table'><tr><td style='line-height:18px;'>"+
	                         "<div class='jw-title'>"+(option.title||'&nbsp;')+"</div></td><td style='width:55px;text-align:right;line-height:18px;'><nobr>";
	             option.max!=false && (header+=_div+" class='max'>□</a></span>");
	             option.close!=false && (header+=_div+" class='close'><img src='"+getContextPath()+"/static/js/jw/icons/close.png'/></a></span>");
	        ...
	        var fn=function(){autoBounds(iw,ih);dialog.is(":visible")&&that.over();};
	        $(win).resize(fn);
    ...
    var jwalert=function(text,option){
    	...
    	var ico=(option.icon+"").split("x"),
    	    ...
    	    code="<div class='jw-alert'>" +
	    			"<div class='icon' style='"+style+"'></div><div class='bd'><table><tr><td style='vertical-align:middle;height:50px;line-height:normal;'>" +(text||'')+ "</td></tr></table></div><div style='clear:both'></div>" +
	    			"<div class='ft'>" +
